...
try {
  ...
  await p.evaluate(() => { try { localStorage.setItem('lss.theme','dark'); } catch(e){} });
  ...
  out.push('review: ' + JSON.stringify(await p.evaluate(() => {
    const de = document.documentElement;
    return {
      step: [...document.querySelectorAll('.step')].map(s => s.textContent.trim() + (s.classList.contains('on') ? '*' : '')),
      rail: document.querySelectorAll('.rail-item').length,
      pane: !!document.querySelector('.content-pane'),
      voiceRow: !!document.querySelector('.anchor-row'),
      panelRadius: getComputedStyle(document.querySelector('.output-grid')).borderRadius,
      sideways: de.scrollWidth > de.clientWidth,
    };
  })));
  await p.screenshot({ path: process.argv[2] });
  // and the same screen in light
  await p.evaluate(() => document.querySelectorAll('.theme-toggle button')[0].click());
  await new Promise(r => setTimeout(r, 800));
  await p.screenshot({ path: process.argv[3] });
  out.push('light bg: ' + await p.evaluate(() => getComputedStyle(document.body).backgroundColor));
