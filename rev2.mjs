...
try {
  ...
  // The app now opens on the Desk board; the intake lives behind "New story".
  ...
  out.push('setup: ' + JSON.stringify(await p.evaluate(() => ({
    setupGrid: !!document.querySelector('.setup-grid'),
    rail: !!document.querySelector('.control-rail'),
    chips: document.querySelectorAll('.chip-toggle').length,
  }))));
  ...
  out.push('review light: ' + JSON.stringify(await p.evaluate(() => ({
    ...
    pane: getComputedStyle(document.querySelector('.content-pane')).backgroundColor,
  }))));
